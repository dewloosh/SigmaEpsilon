...
from ..material.hmh import HMH_M
...
def strain_displacement_matrix_membrane(dshp: ndarray, jac: ndarray) -> ndarray:
    nE = jac.shape[0]
    nP, nN = dshp.shape[:2]
    nTOTV = nN * _NDOFN_
...
@njit(nogil=True, parallel=True, cache=__cache)
def material_strains_membrane(strns: ndarray, z: float) -> ndarray:
    nE, nP = strns.shape[:2]
...
@njit(nogil=True, parallel=True, cache=__cache)
def HMH_membrane(estrs: ndarray) -> ndarray:
    nE, nP = estrs.shape[:2]
...
@njit(nogil=True, parallel=True, cache=__cache)
def model_stiffness_iso_homg_membrane(C: ndarray, t: ndarray) -> ndarray:
    res = np.zeros_like(C)
